// CHATBOT VERSION: 2.1 - Flexible Address Matching
console.log('Tree Grant Chatbot v2.1 loaded - Flexible address matching enabled');
...
function normalizeAddress(address) {
    ...
    // Remove periods, commas, and normalize spaces (be very forgiving)
    normalized = normalized.replace(/[.,]/g, '').replace(/\s+/g, ' ');

    // Expand common abbreviations (more flexible matching)
    const abbreviations = {
        'ST ': 'STREET ',
        'ST$': 'STREET',
        ' AVE': ' AVENUE',
    ...
    // Apply abbreviation expansions with regex for better matching
...
function lookupAddress(address) {
    ...
    console.log('Looking up address:', address, '-> normalized:', normalized);

    // Try exact match first
    const exactMatch = ELIGIBLE_ADDRESSES.find(addr => normalizeAddress(addr) === normalized);
    if (exactMatch) {
        console.log('Exact match found:', exactMatch);
        return { success: true, address: exactMatch };
    }

    // Try partial match (user might have omitted street type)
    // e.g., "2133 W Edgemont" should match "2133 W EDGEMONT AVE"
    const streetTypes = ['ST', 'STREET', 'AVE', 'AVENUE', 'RD', 'ROAD', 'DR', 'DRIVE', 'LN', 'LANE', 'BLVD', 'BOULEVARD', 'PL', 'PLACE', 'CT', 'COURT', 'WAY', 'CIR', 'CIRCLE'];

    for (const eligibleAddr of ELIGIBLE_ADDRESSES) {
        const normalizedEligible = normalizeAddress(eligibleAddr);

        // Check if normalized input is at the start of eligible address
        if (normalizedEligible.startsWith(normalized + ' ')) {
            console.log('Partial match found (with space):', eligibleAddr);
            return { success: true, address: eligibleAddr };
        }

        // Also check if eligible address starts with user input (allows missing street type)
        if (normalizedEligible.startsWith(normalized)) {
            const remainder = normalizedEligible.substring(normalized.length).trim();
            // If the remainder is just a street type, it's a match
            if (streetTypes.includes(remainder)) {
                console.log('Partial match found (street type only):', eligibleAddr, 'remainder:', remainder);
                return { success: true, address: eligibleAddr };
            }
        }

        // Also try: check if we match everything except the street type
        // Split both addresses and compare core parts (number + street name)
        const normalizedParts = normalized.split(' ').filter(p => !streetTypes.includes(p));
        const eligibleParts = normalizedEligible.split(' ').filter(p => !streetTypes.includes(p));

        if (normalizedParts.length > 0 && normalizedParts.join(' ') === eligibleParts.join(' ')) {
            console.log('Core match found (ignoring street type):', eligibleAddr);
            return { success: true, address: eligibleAddr };
        }
    }
    ...
    if (bestScore <= threshold) {
        console.log('Fuzzy match found:', bestMatch, 'score:', bestScore);
...
function startAddressVerification(attemptNumber = 1) {
    // Initialize address attempts if not set
    if (!state.data.addressAttempts) {
        state.data.addressAttempts = 0;
    }

    updateProgress('Step 1: Address Verification', 10);

    let message = attemptNumber === 1
        ? `Perfect! Let's see if your home is in the eligible area. This will just take a second.

Go ahead and enter your street address below:`
        : `Let's try that address again. Make sure to include the street number, direction (N/S/E/W), street name, and street type (Ave/St/Dr/Rd).`;
...
• No apartments or condos with 4+ units
    ...
    if (attemptNumber > 1) {
        message += `<br><em>Attempt ${attemptNumber} of 3</em>`;
    }
    ...
    showTextInput('Enter your address...', (address) => {
        ...
        state.data.addressAttempts = attemptNumber;
        ...
        if (result.success) {
            ...
            if (attemptNumber < 3) {
                addMessage(`Hmm, I couldn't find that address in our eligible list. Don't worry - typos happen! Let's try again.`);
                startAddressVerification(attemptNumber + 1);
            } else {
                showNotEligible();
            }
        }
    ...
    buttons.push({
        ...
        action: () => {
            if (totalTrees === 0) {
                ...
                // Re-show the tree selection with buttons
...
function submitSubmission() {
    ...
    const GOOGLE_SHEETS_URL = 'https://script.google.com/macros/s/AKfycby6zalLjDetf4rrQqcaYxSPD_Y7zJhXZvrsN-7rRNgyt63xxKk9B4kCe6F6OwXnu3ej/exec';
